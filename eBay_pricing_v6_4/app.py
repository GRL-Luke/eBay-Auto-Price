...
def index():
    ...
    if request.method == "POST":
        ...
        # Use your existing filtering pipeline if present; fallback to raw_rows
        rows = data.get("filtered_rows") or raw_rows
        comp_rows = rows

        # Exact-UPC-first pick
        ...
        if comp_rows and user_code:
            ...
            exacts = [r for r in comp_rows if row_code_match(r) and r.get("total") is not None]
            if exacts:
                ebay_exact_row = min(exacts, key=lambda r: r["total"])
                ebay_exact_total = float(ebay_exact_row["total"])

        # If no exact code match, use absolute-low from comp_rows
        ebay_abs_row = None
        ebay_abs_total = None
        if comp_rows:
            valids = [r for r in comp_rows if r.get("total") is not None]
            if valids:
                ebay_abs_row = min(valids, key=lambda r: r["total"])
